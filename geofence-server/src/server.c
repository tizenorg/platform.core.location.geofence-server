...
int _geofence_scan_wifi(GeofenceServer *geofence_server)
{
	FUNC_ENTRANCE_SERVER;
	int ret = 0;
	ret = net_wps_scan_wifi();
	if (ret != NET_ERR_NONE) {
		LOGD_GEOFENCE("Error!! net_wps_scan_wifi() failed. [%d]", ret);
		ret = -1;
	}
	return ret;
}
...
#if 0 /* Not used */
static void create_wifi_info(char *bssid, void **wifi_info)
{
	LOGD_GEOFENCE("create_wifi_info bssid : [%s]", bssid);

	wifi_info_s *info = NULL;
	info = (wifi_info_s *)g_malloc0(sizeof(wifi_info_s));
	g_strlcpy(info->bssid, bssid, 18);

	*wifi_info = (wifi_info_s *) info;
}
#endif
